...
const I = {
  home: (
    <svg viewBox="0 0 24 24" width="28" height="28" fill="none" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round">
  ...
  heart: (
    <svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round">
  ...
  return (
    <>
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 bg-black border-t border-white/10 pb-[env(safe-area-inset-bottom)] z-40">
        <div className="max-w-3xl mx-auto grid grid-cols-6 h-[72px]">
          {items.map(i => {
            const active = path.startsWith(i.href);
            const heartActive = i.accent && active;
            // Convención invertida solicitada: activo = gris, inactivo = ball (verde fluor)
            const colorClass = active
              ? (i.accent ? 'text-red-400' : 'text-white/40')
              : (i.accent ? 'text-red-400/70' : 'text-ball');
            return (
              <Link key={i.href} href={i.href}
                className={`flex flex-col items-center justify-center gap-0.5 text-[11px] font-black transition-transform active:scale-90 ${colorClass}`}>
                <span className={`w-8 h-8 flex items-center justify-center ${heartActive ? 'animate-pulse' : ''}`}>
                  {i.icon}
                </span>

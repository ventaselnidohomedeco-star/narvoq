...
export default function InstallButton({
  variant = 'primary',
  className = ''
}: { variant?: 'primary' | 'ghost' | 'subtle'; className?: string }) {
  ...
  const btnClass = variant === 'primary'
    ...
    : variant === 'subtle'
    ? 'bg-transparent border border-ball/30 text-ball/80 hover:bg-ball/5 font-bold rounded-full px-3 py-1.5 text-xs active:scale-95 transition inline-flex items-center gap-1.5'
  ...
  return (
    <>
      <button onClick={descargar} className={`${btnClass} ${className}`}>
        <svg width={variant === 'subtle' ? 12 : 18} height={variant === 'subtle' ? 12 : 18} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
          <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
          <polyline points="7 10 12 15 17 10" />
          <line x1="12" y1="15" x2="12" y2="3" />
        </svg>
        {variant === 'subtle' ? 'Instalar app' : 'Descargar app'}
      </button>

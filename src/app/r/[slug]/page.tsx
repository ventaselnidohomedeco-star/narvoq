...
export default function ReservarPublico() {
  ...
  // Cargar complejo + canchas
  useEffect(() => {
    (async () => {
      const { data: c } = await supabase.from('complexes')
      ...
      if (!c || !c.active) { setLoading(false); return; }
      setCx(c as Complex);
    ...
    // Auto-guardar precarga de guest para próximas reservas
  ...
  // Recalcular slots al cambiar cancha o fecha
  useEffect(() => {
    if (!cx || !court) return;
    (async () => {
  ...
  async function reservar() {
    ...
    router.push(`/r/${slug}/exito/${bk.id}`);
  }
  ...
  if (!cx) return (
    <main className="min-h-dvh flex flex-col items-center justify-center text-center px-8">
      ...
      <p className="text-white/50 mt-2">Verificá el link o pedile uno nuevo al complejo.</p>
